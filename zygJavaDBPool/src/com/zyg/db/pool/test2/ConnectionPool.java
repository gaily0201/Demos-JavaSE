...
public class ConnectionPool {
	...
	private Connection newConnection() throws SQLException {
		...
		if (connections.size() == 0) {
			...
			//System.out.println("driverMaxConnections: "+driverMaxConnections);
	...
	/**
	 * 本函数从连接池向量 connections 中返回一个可用的的数据库连接，如果当前没有可用的数据库连接，本方法则根据
	 * incrementalConnections设置的值再创建几个数据库连接，并放入连接池中。 如果创建后，所有的连接仍都在使用中，则返回 null
	 * @return 返回一个可用的数据库连接
	 * 
	 */
